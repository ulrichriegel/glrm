...
SimulateClaims <- function(v, m, s_squared, epsilon, NumberOfSimulations) {
  n <- length(v)
  ...
  for (i in 1:n) {
    for (j in 1:n) {
      sigma_sqared <- log(s_squared[j]/(v[i]*m[j]^2)+1)
      mu <- log(v[i]*m[j])-sigma_sqared/2
      sigma <- sqrt(sigma_sqared)
      S[i,j,] <- rlnorm(NumberOfSimulations, meanlog = mu, sdlog = sigma)
    }
  }
...
Generalized_LR_Method_fixed_g_h <- function(S, v, K, g, h, m, s_squared) {
  ###########################################################################################################
