...
NumericVector CreateR_cpp(IntegerVector J_s, IntegerVector Number_of_Est_s, IntegerVector K_s, NumericVector epsilon_s, NumericVector m_s, NumericVector s_squared_s, NumericVector v_s, int n) {
  ...
  k=0;
  for (i=1; i<=n-1; i++) {
    k += Number_of_Est[i] * Number_of_Est[i];
  }
  NumericVector R_vector(k);


  for (i=1; i<=n-1; i++) {
    NumericMatrix R_i(Number_of_Est[i]+1, Number_of_Est[i]+1);

    for (nu =1; nu <= Number_of_Est[i]; nu++) {
      for (mu = nu; mu <= Number_of_Est[i]; mu++) {
        i_1 = J_all(Number_of_Est_cum[i-1] + nu, 1);
        i_2 = J_all(Number_of_Est_cum[i-1] + mu, 1);
        j_1 = J_all(Number_of_Est_cum[i-1] + nu, 2);
        j_2 = J_all(Number_of_Est_cum[i-1] + mu, 2);
        k_1 = J_all(Number_of_Est_cum[i-1] + nu, 3);
        k_2 = J_all(Number_of_Est_cum[i-1] + mu, 3);
        if (j_1 == 0 && j_2 == 0) {
          R_i(nu,mu) = 1/(v[i_1]*v[i_1]) * (prod1plusepssq[i_1]-1);
        } else if (j_1 == 0) {
          R_i(nu,mu) = 1/(v[i_1]*v[i_1]) * (prod1plusepssq[std::max(i_1,j_2)]-1);
        } else if (j_2 == 0) {
          R_i(nu,mu) = 1/(v[i_1]*v[i_1]) * (prod1plusepssq[std::max(j_1,i_2)]-1);
        } else if (j_1 != j_2 && k_1 != k_2) {
          R_i(nu,mu) = 1/(v[i_1]*v[i_1]) * (prod1plusepssq[std::max(j_1,j_2)]-1);
        } else if (j_1 == j_2 && k_1 != k_2) {
          R_i(nu,mu) = 1/(v[i_1]*v[i_1]) * (prod1plusepssq[j_1]-1);
        } else if (j_1 != j_2 && k_1 == k_2) {
          R_i(nu,mu) = 1/(v[i_1]*v[i_1]) * (prod1plusepssq[std::max(j_1,j_2)]*gam(i_1,k_1)-1);
        } else if (j_1 == j_2 && k_1 == k_2) {
          R_i(nu,mu) = 1/(v[i_1]*v[i_1]) * (prod1plusepssq[j_1]*gam(j_1,k_1)*gam(i_1,k_1)-1);
        }
      }
    }

    for (nu =1; nu <= Number_of_Est[i] - 1; nu++) {
      for (mu = nu + 1; mu <= Number_of_Est[i]; mu++) {
        R_i(mu, nu) = R_i(nu,mu);
      }
    }


    for (mu=1; mu <= Number_of_Est[i]; mu++) {
      for (nu=1; nu <= Number_of_Est[i]; nu++) {
        R_vector[Counter] = R_i(nu,mu);
        Counter++;
      }
    }
  }
