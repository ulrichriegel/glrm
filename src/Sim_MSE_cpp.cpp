...
NumericVector Sim_MSE_bullet_GLR_cpp(NumericVector S_sim, NumericVector v_sim, int n, long NumberOfSimulations, NumericVector K_s, NumericVector J_s, IntegerVector Number_of_Est_s, NumericVector h_s, NumericVector g_s, NumericVector v_s, NumericVector m_s, NumericVector s_squared_s) {
  ...
  for (i = 1; i <= n; i++) {
    for (k = 1; k <= Count_K; k++) {
      gam(i,K[k]) = 1+s_squared[K[k]]/(v_true[i]*m[K[k]]*m[K[k]]);
    }
  }
  ...
  NumericMatrix r_hat (n + 1, Max_Number_of_Est + 1);
   ...
   NumericVector ExpectedReserve(n+1);
   NumericVector Reserve_hat(n+1);
   NumericVector SquaredError (3*(n+1));
  ...
  for (SimNr = 1; SimNr <= NumberOfSimulations; SimNr++) {
    ...
    for (i = 2; i <= n; i++) {
      Reserve[i-1] = 0;
      ExpectedReserve[i-1] = 0;
      Reserve_hat[i-1] = 0;
      for (k = n-i+2; k <= n; k++) {
        Reserve[i-1] += S(i,k);
        ExpectedReserve[i-1] += v_true[i] * m[k];
        Reserve_hat[i-1] += v_hat[i] * m_hat[k];
      }
      Reserve[n] += Reserve[i-1];
      ExpectedReserve[n] += ExpectedReserve[i-1];
    ...
    for (i = 0; i <= n; i++) {
      ...
      // random error:
      SquaredError[i+n+1] += (Reserve[i] - ExpectedReserve[i]) * (Reserve[i] - ExpectedReserve[i]);
      // estimation error:
      SquaredError[i+2*(n+1)] += (Reserve_hat[i] - ExpectedReserve[i]) * (Reserve_hat[i] - ExpectedReserve[i]);
...
NumericVector Sim_MSE_bullet_LR_cpp(NumericVector S_sim, NumericVector v_sim, int n, long NumberOfSimulations, NumericVector W_s, NumericVector v_s, NumericVector m_s) {
  ...
  for (SimNr = 1; SimNr <= NumberOfSimulations; SimNr++) {
    ...
    for (i = 0; i <= n; i++) {
      ...
      SquaredError[i + n + 1] += (Reserve[i] - ExpectedReserve[i]) * (Reserve[i] - ExpectedReserve[i]);
      // estimation error
      SquaredError[i + 2*(n+1)] += (ExpectedReserve[i] - Reserve_hat[i]) * (ExpectedReserve[i] - Reserve_hat[i]);
...
NumericVector Sim_MSE_CL_cpp(NumericVector S_sim, int n, long NumberOfSimulations, NumericVector v_s, NumericVector m_s) {
  ...
  for (SimNr = 1; SimNr <= NumberOfSimulations; SimNr++) {
    ...
    f_hat_cum = 1;
    for (i = 2; i <= n; i++) {
      f_hat_cum = f_hat_cum * f_hat[n-i+1];
      Reserve_hat[i-1] = (f_hat_cum - 1) * C(i,n-i+1);
    }
